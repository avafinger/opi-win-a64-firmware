...
dd if=./ub-opi-a64-sdcard.bin conv=notrunc bs=1k seek=19096 of=${SDCARD}
...
tar -xvpzf rootfs_opi-a64_rc1.tar.gz -C ./erootfs --numeric-ow
sync
sudo umount erootfs
rm -fR erootfs
...
tar -xvpzf boot_opi-a64_rc1.tar.gz -C ./eboot --no-same-owner
sync
sudo umount eboot
rm -fR eboot
